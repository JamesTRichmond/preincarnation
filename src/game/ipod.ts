/** Strange Device transport. Hidden <audio> outlives the lid. Files live in IndexedDB. */
...
export function createDeck() {
  ...
  const attach = async (id: string, start: number) => {
    ...
    try {
      audio.currentTime = Math.max(0, Math.min(start, Number.isFinite(audio.duration) ? audio.duration : start));
    } catch {
      /* ignore */
    }
    return true;
  ...
  return {
    ...
    hydrate: async (saved: Partial<DeckMeta> | undefined) => {
      ...
      if (id) {
        const ok = await attach(id, meta.position);
        if (!ok) {
          audio.pause();
          revoke();
